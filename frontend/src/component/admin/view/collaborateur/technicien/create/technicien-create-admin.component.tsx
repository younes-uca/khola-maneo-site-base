...
const TechnicienAdminCreate = () => {

    const [showSavedModal, setShowSavedModal] = useState(false);
    const [showErrorModal, setShowErrorModal] = useState(false);
    const [isTechnicienCollapsed, setIsTechnicienCollapsed] = useState(true);
    ...
    const { control, handleSubmit, reset } = useForm<TechnicienDto>({
        defaultValues: {
        nom: '' ,
        prenom: '' ,
        username: '' ,
    ...
    const handleSave = async (item: TechnicienDto) => {
        Keyboard.dismiss();
        try {
...
return(
    <SafeAreaView style={{ flex: 1, backgroundColor: '#e6e8fa' }} >
        <ScrollView style={{ margin: 20, marginBottom: 80 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" >
            <Text style={{ fontSize: 30, fontWeight: 'bold', alignSelf: 'center', marginBottom: 10 }} >Create Technicien</Text>

            <TouchableOpacity onPress={technicienCollapsible} style={{ backgroundColor: 'orange', padding: 10, borderRadius: 10, marginVertical: 5 }}>
        ...
        <CustomButton onPress={handleSubmit(handleSave)} text={"Save Technicien"} bgColor={'#000080'} fgColor={'white'} />
        </ScrollView>
...
export default TechnicienAdminCreate;
