...
const SiteImageAdminCreate = () => {

    const [showSavedModal, setShowSavedModal] = useState(false);
    const [showErrorModal, setShowErrorModal] = useState(false);
    const [isSiteImageCollapsed, setIsSiteImageCollapsed] = useState(true);
    ...
    const { control, handleSubmit, reset } = useForm<SiteImageDto>({
        defaultValues: {
        site: undefined,
        fileName: '' ,
        filePath: '' ,
    ...
    const handleSave = async (item: SiteImageDto) => {
        ...
        try {
            ...
            console.error('Error saving siteImage:', error);
...
return(
    <SafeAreaView style={{ flex: 1, backgroundColor: '#e6e8fa' }} >
        <ScrollView style={{ margin: 20, marginBottom: 80 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" >
            <Text style={{ fontSize: 30, fontWeight: 'bold', alignSelf: 'center', marginBottom: 10 }} >Create SiteImage</Text>

            <TouchableOpacity onPress={siteImageCollapsible} style={{ backgroundColor: 'orange', padding: 10, borderRadius: 10, marginVertical: 5 }}>
                <Text style={{ textAlign: 'center', fontWeight: 'bold', fontSize: 20 }}>SiteImage</Text>
            </TouchableOpacity>
        ...
        <CustomButton onPress={handleSubmit(handleSave)} text={"Save SiteImage"} bgColor={'#000080'} fgColor={'white'} />
        </ScrollView>
        ...
        {sites !== null && sites.length > 0 ? ( <FilterModal visibility={siteModalVisible} placeholder={"Select a Site"} onItemSelect={onSiteSelect} items={sites} onClose={handleCloseSiteModal} variable={'nom'} /> ) : null}
    </SafeAreaView>
);
};
...
export default SiteImageAdminCreate;
